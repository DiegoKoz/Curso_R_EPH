...
Calculamos el ingreso per capita promedio, utilizando el ponderador correspondiente. **Importante: Debemos exigir ingresos positivos, ya que tenemos numerosos casos con ingreso 0 y otros codificados con  -9 (cuando no corresponde la pregunta por la ocupación principal)** 
```{r, warning=FALSE}
IOppal <-Union_Bases   %>% 
  filter(P21>0) %>% 
  group_by(periodo) %>% 
...
La variable ``` IPCF ``` representa el ingreso per capita del hogar, del individuo encuestado. Su formula es la siguiente: $$\frac{\sum_{i=1}^n ING_i}{n}$$      Donde:   
...
```
Vale mencionar, que al estudiar los ingresos percapita familiares en la base de individuos, el promedio de estos implicitamente pondera más a aquellos hogares con mayor cantidad de individuos (ya que los IPCF de un mismo hogar se encuentrar repetidos en el cálculo tantas veces como personas intengren dicho hogar).         
...
Los gráficos Boxplot, utilizan de manera desagregada cada uno de los valores de una variable. Están compuestos por una caja, cuyo límite inferior es el valor donde se alcanza el 25% de la distribución, y su límite superior el valor donde se alcanza el 75% de la misma. A su vez, también el gráfico marca los valores "outliers" (datos que se encuentran a una distancia de al menos 1,5 veces del límite inferior o superior de la caja, según corresponda)
...
Paramos de un ejemplo sencillo con los ingresos de la ocupación principal. Luego iremos complejizandolo 
```{r warning=FALSE}
